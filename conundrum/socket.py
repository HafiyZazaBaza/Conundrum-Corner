...
@socketio.on("bad_advice_hotline_start_game")
def bad_advice_hotline_start_game(data):
    lobby_code = data.get("lobbyCode")
    ...
    bad_advice_hotline_game_manager.start_game(lobby_code, question, players, host)
    lobby_votes[lobby_code] = {}

    emit("bad_advice_hotline_game_started", {"question": question}, room=lobby_code)
...
def bad_advice_hotline_submit_bad_advice(data):
    ...
    if not lobby_code or bad_advice is None or not bad_advice_hotline_game_manager.games.get(lobby_code):
        emit("error_message", {"message": "Game not found or bad advice missing."}, room=request.sid)
        return
...
def bad_advice_hotline_vote(data):
    ...
    if not bad_advice_hotline_game_manager.games.get(lobby_code):
        emit("error_message", {"message": "Game not started."}, room=request.sid)
        return
...
@socketio.on("obviously_lies_start_game")
def obviously_lies_start_game(data):
    lobby_code = data.get("lobbyCode")
    question = data.get("question")
    ...
    emit("obviously_lies_game_started", {"question": question}, room=lobby_code)
...
def obviously_lies_submit_false_answer(data):
    ...
    if not lobby_code or not obviously_lies_game_manager.games.get(lobby_code):
        emit("error_message", {"message": "Game not found."}, room=request.sid)
        return
...
@socketio.on("reverse_guessing_start_game")
def reverse_guessing_start_game(data):
    lobby_code = data.get("lobbyCode")
    answer = data.get("answer")
    ...
    emit("reverse_guessing_game_started", {"answer": answer}, room=lobby_code)
...
@socketio.on("emoji_translation_start_game")
def emoji_translation_start_game(data):
    lobby_code = data.get("lobbyCode")
    ...
    emit("emoji_translation_game_started", {"emoji_prompt": emoji_prompt}, room=lobby_code)
